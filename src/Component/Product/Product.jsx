                ...
                <div className="product position-relative fs-5 text-muted">
                  {!idOfWishList.find((item) => item === elm._id) ? (
                    <i
                      onClick={() =>
                       ...
                       localStorage.getItem('token')? addToWishList(elm._id):navigator('/login')}
                      className=" px-4 py-2 fa-regular heart  fa-heart"
                    ></i>
                  ) : (
                    <i
                      onClick={() => removeFromWishList(elm._id)}
                      className="px-4 py-2 fa-solid  text-danger heart fa-heart"
                    ></i>
                  )}
                  <Link to={`/product-details/${elm._id}`}>
                    ...
                    <p className="text-main  p-0 m-0">
                      {elm.category.name.split(" ").slice(0, 2).join(" ")}
                    </p>
                  ...
                  <button
                    ...
                    className="btn mb-2  bg-main text-white w-100"
                  >
                    {" "}
                    Add to cart
                  </button>
